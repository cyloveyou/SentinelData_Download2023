    ...
    def Download1(self, DownloadInfo: list):
        ...
        try:
            response = requests.get(
                url, headers=headers, stream=True, proxies=self.proxies, timeout=10
            )
            if response.status_code == 200:
                data_size = (
                    round(float(response.headers["Content-Length"])) / 1024 / 1024
                )
...
if __name__ == "__main__":
    ...
    # 读取urlString
    urlfile = "SearchURL.txt"
    with open(urlfile, mode="r") as f:
        urlString = f.read()  # 网页获取的url,顾及到url过长，故存放在txt文件中

        # 初始化配置
    SL = SentinelDownload(
        UserName=userName, Password=password, Proxies=proxies, SearchUrl=urlString
    )
    # 下载数据
